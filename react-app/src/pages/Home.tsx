...
interface ILandingProps {
  requestAccessHandler: (b: boolean) => void;
}

const Landing = ({ requestAccessHandler }: ILandingProps) => {
  return (
    <>
      ...
      <Button
        sx={{ mt: '1rem', maxWidth: '300px', alignSelf: 'center' }}
        onClick={() => requestAccessHandler(true)}
        variant="contained"
      >
        Request Access
      </Button>
...
export const Home = () => {
  ...
  return (
    <BaseLayout displayFooter>
      ...
      >
        {showAccessForm ? <AccessRequest /> : <Landing requestAccessHandler={setShowAccessForm} />}
      </Box>
    </BaseLayout>
  );
};
