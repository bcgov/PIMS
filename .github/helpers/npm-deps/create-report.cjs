...
const outdatedDeps = require(
	path.resolve(__dirname, `../../../outdatedDeps.json`),
);
...
const outputDepsByVersionChange = (
	...
	results[packagePath] +=
		`${line(`Expand to see individual installs. <br /><br />\n`)}`;
...
const outputDeps = (dependenciesObj, packagePath, isDevDep) => {
	...
	if (isDevDep)
		results[packagePath] +=
			`${heading("Development Dependencies to Update:", 3)}`;
	else
		results[packagePath] +=
			`${heading("Production Dependencies to Update:", 3)}`;
...
(async () => {
	...
	const promises = packageJsonPaths.map(async (packagePath) => {
		...
		if (devDeps.outdated === 0)
			results[packagePath] +=
				`${line(`${check} - Development ${upToDateMsg}`)}`;
		else
			results[packagePath] += `${line(
