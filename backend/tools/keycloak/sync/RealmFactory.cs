    ...
    public class RealmFactory : IRealmFactory
    {
        ...
        private async Task<SKModel.RoleModel> AddUpdateRealmRoleAsync(RoleOptions config)
        {
            ...
            if (role == null)
            {
                ...
                var rRes = await _client.SendRequestAsync(HttpMethod.Post, _client.AdminRoute("roles"), role);
                if (!rRes.IsSuccessStatusCode) throw new HttpResponseException(rRes);
                ...
                if (config.Composite && config.RealmRoles.Any())
                {
                    ...
                    if (!rcRes.IsSuccessStatusCode) throw new HttpResponseException(rcRes);
                }
            }
            else
            {
        ...
        private async Task<SKModel.GroupModel> AddUpdateGroupAsync(GroupOptions config)
        {
            ...
            else
            {
                ...
                if (group.RealmRoles.Any())
                {
                    ...
                    if (!grRes.IsSuccessStatusCode) throw new HttpResponseException(grRes);
                }
            }
        ...
        private async Task<SKModel.ClientModel> AddUpdateClientAsync(ClientOptions config)
        {
            ...
            if (client == null)
            {
                ...
                if (client.ServiceAccountsEnabled)
                {
                    ...
                    if (config.ServiceAccount?.RealmRoles?.Any() ?? false)
                    {
                        ...
                        if (!srRes.IsSuccessStatusCode) throw new HttpResponseException(srRes);
                        ...
                        foreach (var csClient in config.ServiceAccount.ClientRoles ?? Enumerable.Empty<ClientRoleOptions>())
                        {
                            ...
                            if (!scrRes.IsSuccessStatusCode) throw new HttpResponseException(scrRes);
                        }
                    }
                }
            }
            else
            {
                ...
                var cRes = await _client.SendRequestAsync(HttpMethod.Put, _client.AdminRoute($"clients/{client.Id}"), client);
