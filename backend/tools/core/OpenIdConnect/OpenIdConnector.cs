...
namespace Pims.Tools.Core.OpenIdConnect
{
    /// <summary>
    ...
    public class OpenIdConnector : IOpenIdConnector
    {
        ...
        public OpenIdConnector(IOptionsMonitor<OpenIdConnectOptions> optionsTool, IHttpClientFactory clientFactory, ILogger<OpenIdConnector> logger)
        {
            _options = optionsTool.CurrentValue;
            _client = clientFactory.CreateClient("Pims.Tools.Core.OpenIdConnect");
            _logger = logger;
        }
        #endregion
        ...
        public async Task<Models.TokenModel> RequestTokenAsync(string refreshToken = null)
        {
            ...
            request.Headers.Add("User-Agent", "Pims.Tools.Core.OpenIdConnect");
            ...
            {
                var token = await JsonSerializer.DeserializeAsync<Models.TokenModel>(stream, _serializationOptions);
