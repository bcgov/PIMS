...
describe('UNIT - Geoserver services', () => {
  ...
  describe('getSiteAddresses', () => {
    ...
    const fetchMock = jest.spyOn(global, 'fetch').mockImplementation(() => Promise.resolve(new Response(stringjson)));
    ...
    it('should return an error when service is unreachable.', async () => {
      ...
      expect(async () => { await GeocoderService.getSiteAddresses('') }).rejects.toThrow();
    })
  ...
  describe('getPids', () => {
    ...
    it('should get a list of PIDs connected to the site address.', async () => {
      const fetchMock = jest.spyOn(global, 'fetch').mockImplementationOnce(() => Promise.resolve(new Response(stringPids)));
    ...
    it('should thow an error if geocoder service is down.', async () => {
      const fetchMock = jest.spyOn(global, 'fetch').mockImplementationOnce(() => Promise.resolve(new Response('', {status: 500})));
      expect(async () => { await GeocoderService.getPids('') }).rejects.toThrow();
    });
