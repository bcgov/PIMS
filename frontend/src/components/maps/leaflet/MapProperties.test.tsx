...
const mockDetails = {
    ...
    propertyClassification: 'Core Operational',
    description: 'test',
...
const mockParcels = [{id: 1, latitude: 48, longitude: 123}, {id: 2, latitude: 50, longitude: 133}] as IParcel[]
const noParcels = [] as IParcel[];

// Check that the markers have correct po sition given the mock parcel 1 above
it('Renders the marker in correct position', ()=> {
    const component = mount(<Map lat={48.43} lng={-123.37} zoom={14} parcels={mockParcels} activeParcel={mockDetails} onParcelClick={jest.fn()} />);
...
it('Should render 0 markers when there are no parcels', ()=> {
    const component = mount(<Map lat={48.43} lng={-123.37} zoom={14} parcels={noParcels} activeParcel={emptyDetails} onParcelClick={jest.fn()} />);
...
it('Loads parcel details on click', ()=> {
    ...
    const component = shallow(<Map lat={48.43} lng={-123.37} zoom={14} parcels={mockParcels} activeParcel={mockDetails} onParcelClick={onParcelClick} />);
    const marker = component.find(Marker).first();
...
it('ParcelPopupView renders correctly', () => {
    const tree = renderer
    .create(<ParcelPopupView parcelDetail={mockDetails} />)
    .toJSON();
